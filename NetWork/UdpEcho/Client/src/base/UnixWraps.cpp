...
ssize_t UnixWraps::my_read(int fd, char *ptr)
{
...
ssize_t UnixWraps::readline(int fd, void *vptr, size_t maxlen)
{
	ssize_t	n, rc;
	char	c, *ptr;
...
ssize_t UnixWraps::readlinebuf(void **vptrptr)
{
	if (read_cnt)
		*vptrptr = read_ptr;
	return(read_cnt);
...
ssize_t UnixWraps::Readline(int fd, void *ptr, size_t maxlen)
{
	ssize_t		n;

	if ( (n = readline(fd, ptr, maxlen)) < 0)
		errorSys("readline error");
	return(n);
}